...
param_default_converter(const struct param_spec *ps, const cJSON *node, void *value)
{
    ...
    case PARAM_TYPE_I16: {
        ...
        if (to_conv < INT16_MIN || to_conv > INT16_MAX) {
            log_err(
                "Value of %s must be greater than or equal to %d and less than or equal to %d",
                ps->ps_name, INT16_MIN, INT16_MAX);
            return false;
        }
    ...
    case PARAM_TYPE_I32: {
        ...
        if (to_conv < INT32_MIN || to_conv > INT32_MAX) {
            log_err(
                "Value of %s must be greater than or equal to %d and less than or equal to %d",
                ps->ps_name, INT32_MIN, INT32_MAX);
            return false;
        }
...
#endif
        if (to_conv < 0 || (tmp == error && errno == ERANGE) || tmp > SIZE_MAX || *end != '\0') {
            log_err(
    ...
    case PARAM_TYPE_ENUM: {
        ...
        if (to_conv < ps->ps_bounds.as_enum.ps_min || to_conv > ps->ps_bounds.as_enum.ps_max) {
            log_err(
                "Value of %s must be greater than or equal to %d and less than or equal to "
                "%d",
                ps->ps_name, ps->ps_bounds.as_enum.ps_min, ps->ps_bounds.as_enum.ps_max);
            return false;
        }
        *(int *)value = (int)to_conv;
        break;
    ...
    case PARAM_TYPE_STRING:
        if (cJSON_IsNull(node)) {
            ...
            if (n >= ps->ps_bounds.as_string.ps_max_len) {
                log_err(
                    "Length of the value of %s must be less that or equal to %lu", ps->ps_name,
...
param_default_validator(const struct param_spec *ps, const void *value)
{
    ...
    case PARAM_TYPE_INT: {
        ...
        log_err(
            ...
            ps->ps_name, ps->ps_bounds.as_scalar.ps_min, ps->ps_bounds.as_scalar.ps_max);
        break;
    }
    case PARAM_TYPE_I8: {
    ...
    case PARAM_TYPE_U8: {
        ...
        log_err(
            ...
            ps->ps_name, ps->ps_bounds.as_uscalar.ps_min, ps->ps_bounds.as_uscalar.ps_max);
        break;
    }
    case PARAM_TYPE_U16: {
    ...
    case PARAM_TYPE_DOUBLE: {
        ...
        log_err(
            ...
            ps->ps_name, ps->ps_bounds.as_double.ps_min, ps->ps_bounds.as_double.ps_max);
        break;
    }
    case PARAM_TYPE_STRING: {
        if (strnlen(value, ps->ps_bounds.as_string.ps_max_len) < ps->ps_bounds.as_string.ps_max_len)
            return true;
        log_err(
            "Length of the value of %s must be less than or equal to %lu", ps->ps_name,
            ps->ps_bounds.as_string.ps_max_len - 1);
        break;
    }
    ...
    {                                                                                            \
        ...
        case PARAM_TYPE_INT:                                                                     \
            if (tmp < INT_MIN || tmp > INT_MAX) {                                                \
                log_err(                                                                         \
                    "Number of bytes of %s is not within the bounds of a integer", ps->ps_name); \
                return false;                                                                    \
            }                                                                                    \
            *(int *)value = (int)tmp;                                                            \
            break;                                                                               \
        ...
        case PARAM_TYPE_SIZE:                                                                    \
            if (tmp < 0 || tmp > (long double)SIZE_MAX) {                                        \
                log_err(                                                                         \
                    "Number of bytes of %s is not within the bounds of a size_t", ps->ps_name);  \
                return false;                                                                    \
            }                                                                                    \
...
#define STORAGE_JSONIFY(X)                                                       \
    cJSON *param_jsonify_bytes_to_##X(                                           \
        const struct param_spec * const ps, const void * const value)            \
    {                                                                            \
        switch (ps->ps_type) {                                                   \
        case PARAM_TYPE_INT:                                                     \
...
#define STORAGE_STRINGIFY(X)                                                            \
    merr_t param_stringify_bytes_to_##X(                                                \
        const struct param_spec * const ps, const void * const value, char * const buf, \
        const size_t buf_sz, size_t *needed_sz)                                         \
    {                                                                                   \
        int n;                                                                          \
                                                                                        \
        switch (ps->ps_type) {                                                          \
    ...
    ...)
{
    ...
    while (j < num_providers) {
        ...
        err =
            json_walk(provider, params, pspecs_sz, pspecs, ignore_keys_sz, ignore_keys, NULL, true);
        if (err)
            goto va_cleanup;
    ...
    for (size_t i = 0; i < pspecs_sz; i++) {
        ...
        case PARAM_TYPE_STRING:
            if (ps.ps_default_value.as_string) {
                ...
                n = strlcpy(data, ps.ps_default_value.as_string, ps.ps_bounds.as_string.ps_max_len);
