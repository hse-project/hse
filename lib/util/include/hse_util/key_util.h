...
// TODO Gaurav: maybe remove this
/**
 * key_obj_copy_unsafe() - Similar to key_obj_copy(), but assumes that the buffer is large enough.
 *
 * @kbuf:    Buffer into which the key is copied.
 * @kbuf_sz: Size of %kbuf.
 * @klen:    (output) set to length of the stored key.
 ...
 */
static HSE_ALWAYS_INLINE void *
key_obj_copy_unsafe(void *kbuf, size_t kbuf_sz, uint *klen, const struct key_obj *kobj)
{
    uint keylen = kobj->ko_pfx_len + kobj->ko_sfx_len;
