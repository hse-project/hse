    ...
    const merr_t origin)
{
    ...
    err =
        rest_headers_set(resp->rr_headers, REST_HEADER_CONTENT_TYPE, REST_APPLICATION_PROBLEM_JSON);
    ev(err);

    fprintf(
        resp->rr_stream, RFC7807_FMT, reason, status, detail, merr_file(origin),
