...
static void *ml_writer(void *arg)
{
	...
	if (!resp) {
		...
		fprintf(stderr, "[%d]%s: Unable to allocate response struct:%s\n", id,
			__func__, merr_strinfo(err, err_str, sizeof(err_str), NULL));
		return resp;
	}
...
static merr_t perf_seq_writes(const char *mpname, const struct hse_params *params)
{
	merr_t err = 0;
	u32    tc;
	int    i;
	...
	for (i = 0; i < tc; i++) {
		...
		/* Create an mdc */
		err = mpool_mdc_alloc(mp, 0xaabbcc00 + i, capacity, mclass,
                              &oid[i].oid[0], &oid[i].oid[1]);
		if (err) {
			fprintf(stderr, "[%d]: Unable to alloc mdc: %s\n", i,
				merr_strinfo(err, err_str, sizeof(err_str), NULL));
			goto free_oid;
		}
		...
		if (err) {
			fprintf(stderr, "[%d]: Unable to commit mdc: %s\n", i,
				merr_strinfo(err, err_str, sizeof(err_str), NULL));
			goto free_oid;
		}
	...
	printf("%d threads wrote %ld bytes in %d usecs or %4.2f MB/s\n",
		tc, (long)bytes_written, usec, perf);
...
merr_t perf_seq_reads(const char *mpname, const struct hse_params *params)
{
	return perf_seq_writes(mpname, params);
}

struct option longopts[] = {
    { "recsz",   required_argument, NULL, 'r' },
