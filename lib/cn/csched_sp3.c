...
sp3_submit(struct sp3 *sp, struct cn_compaction_work *w, uint qnum)
{
    ...
    if (debug_sched(sp) || (w->cw_debug & CW_DEBUG_START)) {
        ...
        int running_jobs = sp->jobs_started - sp->jobs_finished;

        // clang-format off
        log_info(
            "job=%u jcnt=%d bcnt=%u qnum=%u reduce=%d "
            "cnid=%lu comp=%s rule=%s nodeid=%lu "
            "c_nk=%u c_nv=%u c_kvsets=%u "
            "nd_kvsets=%u nd_keys=%lu nd_hll%%=%lu nd_clen_mb=%lu "
            "samp=%u",
            w->cw_job.sj_id, running_jobs, busycnt, w->cw_qnum, sp->samp_reduce,
...
sp3_ucomp_check(struct sp3 *sp)
{
     ...
     */
    if (sp->ucomp_active) {
        const bool no_ongoing_jobs = sp->jobs_started == sp->jobs_finished;
        const bool completed =
            sp->ucomp_canceled || (!sp->samp_reduce && (!sp->ucomp_full || no_ongoing_jobs));
