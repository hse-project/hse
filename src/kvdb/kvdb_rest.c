    ...
    void *            context)
{
    ...
    b += snprintf_append(buf, bufsz, &buf_off, "request_cancelled: %u\n", status.kvcs_cancelled);
...
kvdb_rest_register(const char *mp_name, void *kvdb)
{
    ...
    status = rest_url_register(kvdb, URL_FLAG_NONE, rest_kvdb_get, 0, "mpool/%s", mp_name);
...
kvs_rest_register(const char *mp_name, const char *kvs_name, void *kvs)
{
    ...
    if (!mp_name || !kvs_name || !kvs)
        return merr(ev(EINVAL));

    status = rest_url_register(
        kvs, URL_FLAG_NONE, rest_kvs_tree, 0, "mpool/%s/kvs/%s/cn/tree", mp_name, kvs_name);

    if (ev(status) && !err)
        err = status;

    status = rest_url_register(
        kvs, URL_FLAG_BINVAL, rest_kvs_curperf, 0, "mpool/%s/kvs/%s/curperf", mp_name, kvs_name);
...
kvs_rest_deregister(const char *mp_name, const char *kvs_name)
{
    ...
    status = rest_url_deregister("mpool/%s/kvs/%s/cn/tree", mp_name, kvs_name);

    if (ev(status) && !err)
        err = status;

    status = rest_url_deregister("mpool/%s/kvs/%s/curperf", mp_name, kvs_name);
