...
usage(void)
{
    ...
    printf("Key/value count and format:\n"
           ...
           "  -m, --mixed        do transaction and non-tx tests\n"
           "  -f, --pfxlen       prefix len\n"
           "Phases:\n"
           "  -p, --put       put keys\n"
           "  -u, --up        update keys\n"
    ...
    printf("Mandatory parameters:\n"
           ...
           "\n"
           "Examples:\n"
           "  c1ingest mp1 db1 kvs2\n"
           "  c1ingest mp1 db1 kvs%%d\n"
           "  c1ingest mp1 db1 foo:bar:baz\n"
           "  c1ingest mp1 db1 kvs3 -t8 -c1000 "
...
int
c1ingest_parse(int argc, char **argv)
{
    int       last_arg;
    int       rc;
    hse_err_t err;
    ...
    options_default(&opt);
    rc = options_parse(argc, argv, &opt, &last_arg);
    if (rc != 0)
        return rc;
