...
static void
_error_quit(const char *detail, u64 err, const char *file, int line)
{
    char err_buf[300];
...
strerror(int errnum)
{
    ...
    strerror_r(errnum, tls_errbuf, sizeof(tls_errbuf));
    return tls_errbuf;
}
...
void
test_put_impl(struct thread_info *ti, uint salt, u64 time)
{
    hse_err_t err;
    u64       tmelapsed = 0;
    u64       i;
    ...
    for (i = opt.kstart;; i++) {
        ...
        err = hse_kvs_put(
            kvs_h[idx], NULL, (char *)ti->ref_key, ti->ref_klen, (char *)ti->ref_val, ti->ref_vlen);
        if (err)
