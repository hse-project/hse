...
#define merr_quit(detail, err) quit("%s:%d: %s: %ld", __FILE__, __LINE__, (detail), (err));
...
test_put(struct thread_info *ti, uint salt)
{
    ...
    for (i = opt.kstart; i < last_key; i++) {
        ...
        rc = hse_kvs_put(ti->kvs, NULL, ti->ref_key, ti->ref_klen, ti->ref_val, ti->ref_vlen);
        if (rc)
...
test_delete(struct thread_info *ti)
{
    ...
    if (opt.do_pdel) {
        ...
        if (plen)
            fprintf(
                stderr,
                "pfxlen (%d) did not match kvs pfxlen "
                "(%lu)\n",
                ti->pfxlen,
                plen);
...
test_put_verify(struct thread_info *ti, uint salt)
{
    ...
    test_start_phase(ti, salt ? "Verify updated keys" : "Verify newly inserted keys");
