...
ctxn_validation_perf(struct tdargs *tdargs)
{
    ...
    for (i = 0; i < keymax; ++i) {
        ...
        if (mode_put) {
            ...
            rc = hse_kvs_get(kvs, &os, key, klen, &found, &vtxn, sizeof(vtxn), &vtxnlen);
            if (rc)
...
ctxn_validation_stress(struct tdargs *tdargs)
{
    ...
    for (i = 0; i < keymax; ++i) {
        ...
        rc = hse_kvs_get(kvs, NULL, key, klen, &found, &vcur, sizeof(vcur), &vcurlen);
        if (rc)
    ...
    for (i = 0; i < keymax; ++i) {
        ...
        rc = hse_kvs_get(kvs, NULL, key, klen, &found, &val, sizeof(val), &vlen);
        if (rc)
...
ctxn_validation_basic(void)
{
    ...
    if (mixed_sz) {
        ...
        rc = hse_kvs_put(kvs, &os, key_lg, klen_lg, &val_lg, sizeof(val_lg));
        if (rc)
...
spawn(spawn_cb_t *func)
{
    ...
    for (i = 0; i < jobsmax; ++i) {
        ...
        if (rc) {
            eprint("only able to create %d of %lu jobs: %s", i, jobsmax, strerror(rc));
            break;
        }
    }
...
usage(void)
{
    ...
    printf("-i iters    specify max number of test iterations\n");
...
main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        c = getopt(argc, argv, ":cf:hi:j:K:k:p:rS:sVvwm");
        if (-1 == c)
            break;
    ...
    if (verbosity > 0) {
        ...
        printf("%12lu  majflt\n", rusage.ru_majflt);
        printf("%12lu  minflt\n", rusage.ru_minflt);
        printf("%12.6lf  test elapsed secs\n", usecs / 1000000.0);
        printf("%12.6lf  open elapsed secs\n", (stats.tstart - stats.topen) / 1000000000.0);
        printf("%12.6lf  close elapsed secs\n", (stats.tclose - stats.tstop) / 1000000000.0);
        printf("%12.6lf  total elapsed secs\n", (stats.tclose - stats.topen) / 1000000000.0);
    }
