...
from bokeh.models import Ellipse, MultiLine, GraphRenderer, StaticLayoutProvider, Slider, CustomJS
    ...
    def sum_by_path(self, pivot=2, param_name='nkvsets'):
        ...
        def traverse_recursive(tn, param_sum=0, param_name='nkvsets'):
            if tn.level < pivot:
                ...
                for c in tn.children:
                    kv_sum, path_cnt = traverse_recursive(c, param_sum + tn.node_info[param_name])
                    sum_tot += kv_sum
...
class TreeShape:
    ...
    def construct_tree(self, ybuf):
        ...
        for n in ybuf['nodes']:
            ...
            tree_dict[(level, offset)] = TreeNode(level=level, offset=offset, node_info=n['info'])
    ...
    def create_slider(self):
        ...
        slider = Slider(start=0, default_size=self.fig_width, end=last, value=last, step=1, title='Tree shape timeline')
        callback = CustomJS(args=dict(graph=self.graph, nrd_list=self.nrd_list, erd_list=self.erd_list, graph_layout_list=self.graph_layout_list), code="""
            var g = graph
    ...
    def plot_graph(self, title):
        ...
        self.graph.node_renderer.glyph = Ellipse(
                height=self.el_size, width=self.el_size,
                fill_alpha=0.8, fill_color='color')
        ...
        plot = figure(title=title,
                x_range=(-self.max_dim, self.max_dim), y_range=(-self.max_dim, self.max_dim),
