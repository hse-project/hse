...
def parse_cmdline():
    desc = 'Plot Crossover data\n\n'\
           'example: %(prog)s --ouput_file=/tmp/plot.html --test_dir=/var/tmp/test\n'\

    p = argparse.ArgumentParser(description = desc,
                                formatter_class=argparse.RawTextHelpFormatter)
    p.add_argument('-o', '--out_file', type=str, help='html output file', required=False);
...
def make_plots(arg_dict):
    ...
    title = 'vlen: {} nkey: {} size: {}'.format(vlen, u.humanize(nkey), u.humanize(nkey * vlen))
    heading = Div(text=f'''
    <hr style="width:100%">
    <h2>{title}</h2>
    ''', width=1000)
    ...
    tree_plot, slider = ts.plot_graph(title='nkvsets Get: {} Cursor: {} Total: {}'.format(g_kvsets_avg, c_kvsets_avg, tot_kvsets))

    df = c.df_for_nkeys(nkey_dir=crossover_path, c_avg_nkvset=c_kvsets_avg, g_avg_nkvset=g_kvsets_avg)
