...
def main():
    ...
    for kvs in kvslist:
        ...
        url[kvs] = 'http+unix://%s/mpool/%s/kvs/%s/cn/tree' %\
            (socket_path.replace('/', '%2F'), mpname, kvsname)
    ...
    while True:
        ...
        for kvs in kvslist:
            ...
            if os.path.exists(sockpath[kvs]):
                ...
                print('[%s] Iteration #%06d of KVS %s path %s does not exist, '
                      'KVS not open? Skipping.' %
                      (dt(), counter, kvs, sockpath[kvs]))
                continue

            if response.text.startswith('Usage:'):
                print('[%s] Iteration #%06d of KVS %s returned usage message, '
