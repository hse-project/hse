...
struct option longopts[] = { { "dryrun", no_argument, NULL, opt_dryrun },
...
void
opts_parse(int argc, char **argv, struct opts *opt, int *last_arg)
{
    int done;
...
gtod_usec(void)
{
    ...
    return (u64)ctime.tv_sec * (u64)1000000 + (u64)ctime.tv_usec;
}
...
xkvs_get(struct test *t, void *key, size_t klen, bool *found, void *val, size_t *vlen)
{
    ...
    *vlen = MAX_VMAX; /* assign here so valgrind knows it's initialized */
    return hse_kvs_get(t->kvs_h, 0, key, klen, found, val, *vlen, vlen);
}
...
{
    return opt.dryrun ? 0UL : hse_kvs_prefix_delete(t->kvs_h, 0, key, klen, 0);
}
...
xkvdb_scan_begin(struct test *t, void *pfx, size_t plen, void **cur)
{
    ...
    if (opt.dryrun)
        ...
        return hse_kvs_cursor_create(t->kvs_h, 0, pfxkey, pfxlen, (struct hse_kvs_cursor **)cur);
}
...
void
do_puts(struct tstate *ts, u64 first_key, u64 num_keys, char *key_range_name, char tag)
{
    u64 keynum;
    u32 vlen;
    u32 klen;
...
void
do_deletes(struct tstate *ts, u64 first_key, u64 num_keys, char *key_range_name)
{
    u64 i;
    u64 keynum;
    u32 klen;
    ...
    int            found)
{
    ...
    if (!verify_delete)
        fprintf(
            stderr,
            "Error #%d: expected %u byte value: %s\n",
            errors,
            explen,
        ...
        fprintf(
            stderr,
            "keynum %lu puts %d upds %d dels %d\n",
            keynum,
            keystats[keynum].ks_puts,
            keystats[keynum].ks_upds,
            keystats[keynum].ks_dels);
    ...
    if (found)
        fprintf(
            stderr,
            "Error #%d: retrieved %u byte value: %s\n",
            errors,
            vlen,
...
void
do_cursor_verify(struct tstate *ts, u64 d1_len, u64 u_len, u64 p_len)
{
    int         err;
...
void
do_verifies(struct tstate *ts, u64 first_key, u64 num_keys, char *key_range_name, char tag)
{
    u64         rc;
    void *      key, *exp;
    size_t      klen, vlen;
...
void
get_stats(struct test_stats curr[MAX_THREADS], struct test_stats *tot, u64 *elapsed_time)
{
    u32 i;
...
main(int argc, char **argv)
{
    ...
    err = hse_parse_cli(argc - last_arg, argv + last_arg, &last_arg, 0, opt.params);
    if (err)
