...
fatal(int err, const char *fmt, ...)
{
    ...
    fprintf(stderr, "%s: %s%s%s\n", progname, msg, err ? ": " : "", err ? strerror(err) : "");
...
simple_client(const char *mp_name, const char *kvs_name, bool read, bool write, bool verbose)
{
    ...
    for (i = 0; read && i < kmax; i++) {
        ...
        err = hse_kvs_get(kvs, NULL, key, klen, &found, val, sizeof(val), &vlen);
        if (err)
        ...
        if (xlen != vlen)
            fatal(EINVAL, "value length mismatch: key %s, exp %d, got %d", key, xlen, vlen);
