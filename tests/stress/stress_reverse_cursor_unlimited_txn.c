...
cursor_read(void *args)
{
    ...
    if (hse_err) {
        ...
        log_print(
            LOG_ERROR, "hse_kvs_cursor_create: errno=%d msg=\"%s\" rank=%d",
            hse_err_to_errno(hse_err), msg, info->rank);
        ++error_count;
        goto out;
    } else if (DEBUG) {
    ...
    for (i = info->end - 1; i >= info->start; i--) {
        ...
        if (i == info->end - 1) {
            hse_err = hse_kvs_cursor_seek(CURSOR, 0, expected_key_buf, info->key_size, NULL, NULL);
            ...
            if (hse_err) {
                ...
                log_print(
                    ...
                    hse_err_to_errno(hse_err), msg, info->rank, i, expected_key_buf);
                ++error_count;
                break;
            } else if (DEBUG) {
                log_print(
                    LOG_DEBUG, "hse_kvs_cursor_seek: rank=%d key_index=%ld key=\"%s\"", info->rank,
                    i, expected_key_buf);
            }
        }

        hse_err = hse_kvs_cursor_read(CURSOR, 0, &cur_key, &cur_klen, &cur_val, &cur_vlen, &eof);

        if (hse_err) {
            hse_strerror(hse_err, msg, sizeof(msg));
            log_print(
                LOG_ERROR, "hse_kvs_cursor_read: errno=%d msg=\"%s\" rank=%d key_index=%ld",
                hse_err_to_errno(hse_err), msg, info->rank, i);
            ++error_count;
            break;
        } else if (DEBUG) {
            log_print(
        ...
        if (eof) {
            log_print(
                ...
                info->rank, info->start, info->end, i);
    ...
    if (hse_err) {
        hse_strerror(hse_err, msg, sizeof(msg));
        log_print(
            LOG_ERROR, "hse_kvs_cursor_destroy: errno=%d msg=\"%s\" rank=%d",
            hse_err_to_errno(hse_err), msg, info->rank);
        ++error_count;
    } else if (DEBUG) {
...
point_insertion(void *args)
{
    ...
    log_print(
        LOG_INFO, "begin %s: rank=%d start=%ld end=%ld", __func__, info->rank, info->start,
        info->end);
    ...
    for (i = info->start; i < info->end; i++) {
        ...
        while (txn_size_idx < info->txn_size) {
            ...
            generate_record(
                ...
                info->data, i + txn_size_idx);

            do {
                err = hse_kvs_put(
                    info->kvs, 0, txn, key_buf, sizeof(key_buf), val_buf, sizeof(val_buf));
                ...
                if (hse_err_to_errno(err) == EAGAIN) {
                    ...
                    if (err) {
                        hse_strerror(err, msg, sizeof(msg));
                        log_print(
                            LOG_ERROR, "hse_kvdb_txn_abort: errno=%d msg=\"%s\"",
                            hse_err_to_errno(err), msg);
                        ++error_count;
                        goto out2;
                    }
                    ...
                    if (err) {
                        hse_strerror(err, msg, sizeof(msg));
                        log_print(
                            LOG_ERROR, "hse_kvdb_txn_begin: errno=%d msg=\"%s\"",
                            hse_err_to_errno(err), msg);
                        ++error_count;
                        goto out2;
                    }
                    ...
                    hse_strerror(err, msg, sizeof(msg));
                    log_print(
                        LOG_ERROR, "hse_kvs_put: errno=%d msg=\"%s\" key=\"%s\"",
                        hse_err_to_errno(err), msg, key_buf);
                    ...
                    if (err) {
                        hse_strerror(err, msg, sizeof(msg));
                        log_print(
                            LOG_ERROR, "hse_kvdb_txn_abort: errno=%d msg=\"%s\" i=%ld",
                            hse_err_to_errno(err), msg, i);
                        ++error_count;
                        goto out2;
                    } else if (DEBUG) {
    ...
    commit:
        ...
        if (err) {
            hse_strerror(err, msg, sizeof(msg));
            log_print(
                LOG_ERROR, "hse_kvdb_txn_commit: error=%d msg=\"%s\" rank=%d",
                hse_err_to_errno(err), msg, info->rank);
...
out:
    log_print(
        LOG_INFO, "end %s: rank=%d start=%ld end=%ld", __func__, info->rank, info->start,
        info->end);
...
spawn_threads(struct cursor_test_data *params, void *thread_fun, char *fun_name)
{
    ...
    log_print(
        LOG_INFO, "completed wait for %d spawned thread(s), fun_name=\"%s\"", thread_count,
        fun_name);
}
