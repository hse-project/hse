...
def separate_keys(kvdb: hse.Kvdb, kvs: hse.Kvs):
    ...
    with kvdb.transaction() as t:
        # Cursor
        with kvs.cursor(filt=b"ab", txn=t, flags=hse.CursorFlag.REVERSE | hse.CursorFlag.BIND_TXN) as c:
            # Read all keys
...
def ptombs_lc_cn(kvdb: hse.Kvdb, kvs: hse.Kvs, cursor_sync: bool=False):
    ...
    with kvdb.transaction() as t:
        # Cursor
        with kvs.cursor(filt=b"ab", flags=hse.CursorFlag.REVERSE | hse.CursorFlag.BIND_TXN) as c:
            if cursor_sync:
                kvdb.sync()
