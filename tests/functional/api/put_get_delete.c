...
{
    tup->keylen = snprintf(tup->key, sizeof(tup->key), "%s%d", prefix, id);
    ASSERT_LT_RET(tup->keylen, sizeof(tup->key), -1);
...
MTF_DEFINE_UTEST(put_get_delete, put_get)
{
    ...
    for (int i = -10; i <= 10; i++) {
        ...
        err = hse_kvs_put(kvs, NULL, tup.key, tup.keylen, tup.putval, tup.vlen);
        ASSERT_EQ(err, 0);
    }
}
...
MTF_DEFINE_UTEST(put_get_delete, kvs_put_get_delete_testcase)
{
    ...
    err = hse_kvs_put(kvs, NULL, test_key, keylen, test_value, vallen);
    ASSERT_EQ(err, 0);

    /* TC: A KVS get with an existing key can be found and will return a correct value */
    found = false;
    err = hse_kvs_get(kvs, NULL, test_key, keylen, &found, vbuf, sizeof(vbuf), &vlen);
    ASSERT_EQ(err, 0);
    ...
    found = false;
    err = hse_kvs_delete(kvs, NULL, test_key, keylen);
    ASSERT_EQ(err, 0);
    err = hse_kvs_get(kvs, NULL, test_key, keylen, &found, vbuf, sizeof(vbuf), &vlen);
    ASSERT_EQ(err, 0);
