...
MTF_DEFINE_UTEST(mclass_policy_test, incorrect_schema)
{
    ...
    ASSERT_NE(err, 0);
    err = kvdb_rparams_from_paramv(&params, NELEM(paramv_age_unknown_key), paramv_age_unknown_key);
    ASSERT_NE(err, 0);
    err = kvdb_rparams_from_paramv(
    ...
    ASSERT_NE(err, 0);
    err = kvdb_rparams_from_paramv(&params, NELEM(paramv_dtype_schema1), paramv_dtype_schema1);
    ASSERT_NE(err, 0);
    err = kvdb_rparams_from_paramv(&params, NELEM(paramv_dtype_schema2), paramv_dtype_schema2);
    ASSERT_NE(err, 0);
    err = kvdb_rparams_from_paramv(&params, NELEM(paramv_dtype_schema3), paramv_dtype_schema3);
    ASSERT_NE(err, 0);
    err = kvdb_rparams_from_paramv(&params, NELEM(paramv_dtype_schema4), paramv_dtype_schema4);
    ASSERT_NE(err, 0);
}
