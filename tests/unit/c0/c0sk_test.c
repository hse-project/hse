...
no_fail_pre(struct mtf_test_info *info)
{
    ...
    mapi_inject(mapi_idx_kvdb_ctxn_set_wait_commits, 0);
...
MTF_DEFINE_UTEST_PREPOST(c0sk_test, ingest, no_fail_pre, no_fail_post)
{
    ...
    err = c0kvms_create(1, &seqno, NULL, &kvms);
    ASSERT_EQ(0, err);
    ASSERT_NE(NULL, kvms);
...
MTF_DEFINE_UTEST_PREPOST(c0sk_test, ingest_debug, no_fail_pre, no_fail_post)
{
    ...
    err = create_mock_cn(&mock_cn, false, false, pfx_len);
    ASSERT_EQ(0, err);
    ...
    for (i = 0; i < 1000; ++i) {
        ...
        err = c0sk_get(mkvdb.ikdb_c0sk, skidx, pfx_len, &kt, atomic_read(&seqno), 0, &res, &vbuf);
        ASSERT_EQ(0, err);
...
MTF_DEFINE_UTEST_PREPOST(c0sk_test, open_test, no_fail_pre, no_fail_post)
{
    ...
    for (i = 0; i < num_kvs; i++) {
        err = c0kvms_create(1, &seqno, NULL, &kvms[i]);
        ASSERT_EQ(0, err);
...
ctxn_trylock(int num)
{
    ...
    return atomic_cas(&ctxn_locks[num], 0, 1);
}
...
ctxn_unlock(int num)
{
    ...
    b = atomic_cas(&ctxn_locks[num], 1, 0);
    assert(b);
}
...
#define nkeys (100 * 1000)
    ...
    for (i = 0; i < 5; ++i) {
        ...
        err = c0sk_cursor_create(c0sk, atomic_read(&seqno), skidx, 0, 0, "", 0, &summary, &cur[i]);
        ASSERT_EQ(0, err);
        atomic_inc(&seqno);
        ...
        for (; j < n; ++j) {
            ...
            vlen = snprintf(vbuf, sizeof(vbuf), "%lu", (ulong)atomic_read(&seqno));
    ...
    err = c0sk_cursor_update(cur[0], atomic_read(&seqno), 0);
    ASSERT_EQ(0, err);
...
MTF_DEFINE_UTEST_PREPOST(c0sk_test, c0_cursor_eagain, no_fail_pre, no_fail_post)
{
    ...
    err = c0sk_cursor_create(c0sk, atomic_read(&seqno), skidx, 0, 0, "", 0, &summary, &cur);
    ASSERT_EQ(0, err);
    ...
    for (i = 0; i <= HSE_C0_KVSET_CURSOR_MAX; ++i) {
        ...
        c0sk_sync(c0sk, HSE_KVDB_SYNC_ASYNC);
        err = c0sk_cursor_update(cur, atomic_fetch_add(&seqno, 1), 0);
        if (err)
...
MTF_DEFINE_UTEST_PREPOST(c0sk_test, c0_cursor_ptombs, no_fail_pre, no_fail_post)
{
    ...
    err = create_mock_cn(&cn, false, false, 4);
    ASSERT_EQ(0, err);
